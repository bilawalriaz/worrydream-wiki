...
*   **Backus 1978 (FP - Can Programming Be Liberated from the von Neumann Style?):** Both papers advocate for a radical shift in programming paradigm based on a clean, formal model (functional programming for Backus, generative communication for Carriero/Gelernter). Both criticize the prevailing imperative/modular style for complicating reasoning. However, they propose orthogonal solutions: Backus focuses on computation (eliminating side effects), while Linda focuses on coordination (managing side effects across processes).
